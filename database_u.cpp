...
bool Database_u::AddUser(QString login, QString password) // Добавление пользователя в базу данных (сохраняются логин и пароль)
{
    ...
    if (!CheckLogin(login)) { // Если соблюдены все условия для регистрации, то данные сохраняются
...
bool Database_u::CheckLogin(QString login) // Проверка наличия логина в базе данных
{
    QSqlQuery *query = new QSqlQuery(db);
    query->prepare("SELECT * FROM users WHERE Login=:Login LIMIT 1");
    query->bindValue(":Login", login);
    query->exec();
    return query->next();
}
